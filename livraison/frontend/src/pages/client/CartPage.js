...
export default function CartPage() {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      {/* Steps */}
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '2rem', gap: '.5rem' }}>
        {[['1','Panier'],['2','Livraison'],['3','Confirmation']].map(([n, l], i) => (
          <React.Fragment key={n}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '.4rem' }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: step >= i+1 ? '#2563eb' : '#e5e7eb', color: step >= i+1 ? '#fff' : '#9ca3af', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.8rem', fontWeight: 700 }}>{n}</div>
              <span style={{ fontSize: '.85rem', fontWeight: step === i+1 ? 600 : 400, color: step === i+1 ? '#1f2937' : '#9ca3af' }}>{l}</span>
            </div>
            {i < 2 && <div style={{ flex: 1, height: 2, background: step > i+1 ? '#2563eb' : '#e5e7eb' }} />}
          </React.Fragment>
        ))}
      </div>
          ...
          {items.length === 0 ? (
            ...
            <>
              <div className="card" style={{ marginBottom: '1rem' }}>
                {items.map((item, i) => (
                  <div key={item.produit} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem 1.25rem', borderBottom: i < items.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                    <div style={{ width: 48, height: 48, background: '#f3f4f6', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>📦</div>
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontWeight: 700, minWidth: 80, textAlign: 'right' }}>{(item.prix * item.quantite).toFixed(2)} MAD</div>
